...
La estructura de datos \textit{inode} definida en el archivo \path{include/linux/fs.h} es la que se encarga de guardar información acerca de los archivos. Esta estructura no guarda información acerca del nombre del archivo ya que esta información se almacena en el \textit{dentry}.~(\cite{inode:wikipedia})
